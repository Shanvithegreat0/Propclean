    ...
    if (user) {
        ...
        onValue(userRef, (snapshot) => {
            ...
            if (userData) {
                ...
                fetchAndDisplayTasks(tasksRef, userOffice);
            } else {
...
function fetchAndDisplayTasks(tasksRef, userOffice) {
    onValue(tasksRef, function(snapshot) {
        if (snapshot.exists()) {
            ...
            tasksArray.forEach(function(taskItem) {
                ...
                appendTaskToShoppingListEl(taskItem[0], taskData, userOffice);
            });
        } else {
...
function appendTaskToShoppingListEl(taskId, taskData, userOffice) {
    let newEl = document.createElement("li");
    newEl.textContent = `${taskData.task} - ${taskData.time || 'No time specified'}`;
    newEl.addEventListener("click", function() {
        window.location.href = `taskDetails.html?id=${taskId}&office=${userOffice}`;
    });
